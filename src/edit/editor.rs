...
impl Editor {
    ...
    fn save(&mut self) {
        if self.file.file_name.is_none() {
            ...
            self.file.file_name = new_name;
        }

        if self.file.save().is_ok() {
    ...
    fn process_keypress(&mut self) -> Result<(), std::io::Error> {
        ...
        match pressed_key {
            Key::Ctrl('q') => {
                if self.quit_times > 0 && self.file.is_dirty() {
    ...
    fn move_cursor(&mut self, key: Key) {
        ...
        let mut width = self.file.row(y).map_or(0, |row: &Row| row.len());
        ...
        width = self.file.row(y).map_or(0, |row: &Row| row.len()); 
    ...
    fn draw_welcome_message(&self) {
        let mut welcome_message = format!("Cell editor -- version {}", VERSION);
    ...
    fn draw_status_bar(&self) {
        let mut status;
        let width = usize::from(self.terminal.size().width);
        let modified_indicator = if self.file.is_dirty() {
            " (modified)"
        } else {
            ""
        };
